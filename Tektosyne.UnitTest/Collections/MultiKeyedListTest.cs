...
namespace Tektosyne.UnitTest.Collections {
    ...
    using Int32StringList = MultiKeyedList<Int32, KeyedType<Int32>>;

    using StringKeyed = KeyedType<String>;
    using StringStringList = MultiKeyedList<String, KeyedType<String>>;

    [TestFixture]
    public class MultiKeyedListTest {
        ...
        [Test]
        public void KeyConverter() {
            _list.KeyConverter = item => item.Text;
            Assert.AreEqual(new StringKeyed("two", "second value"), _list["second value"]);
        }
